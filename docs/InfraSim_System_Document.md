# InfraSim - 仮想インフラ カオスエンジニアリング シミュレータ
...
InfraSimは、実際のインフラに一切触れることなく、インフラ障害の連鎖的影響をシミュレーションする仮想カオスエンジニアリングツールである。インフラの依存関係グラフをモデル化し、150以上のカオスシナリオを自動生成・実行して、システムの脆弱性と障害連鎖リスクを可視化する。
...
| コマンド | 説明 | 主要オプション |
|---------|------|------|
| **infrasim scan** | ローカルシステム/Prometheusからインフラ検出 | --output, --hostname, --prometheus-url |
| **infrasim load** | YAMLファイルからインフラ読み込み | YAML_FILE, --output |
| **infrasim tf-import** | Terraform stateからインポート | --state, --dir, --output |

### シミュレーション

| コマンド | 説明 | 主要オプション |
|---------|------|------|
| **infrasim simulate** | カオスシミュレーション実行 | --model, --html |
| **infrasim demo** | デモインフラでシミュレーション実行 | --web, --host, --port |
| **infrasim tf-plan** | Terraform planの変更影響分析 | PLAN_FILE, --dir, --html |

### レポート・表示

| コマンド | 説明 | 主要オプション |
|---------|------|------|
| **infrasim show** | インフラモデルのサマリー表示 | --model |
| **infrasim report** | HTMLレポート生成 | --model, --output |
| **infrasim serve** | Webダッシュボード起動 | --model, --host, --port |

### セキュリティフィード

| コマンド | 説明 | 主要オプション |
|---------|------|------|
| **infrasim feed-update** | セキュリティニュースからシナリオ自動生成 | --model, --timeout |
| **infrasim feed-list** | 保存済みフィードシナリオ一覧表示 | なし |
| **infrasim feed-sources** | 設定されたフィードソース一覧 | なし |
| **infrasim feed-clear** | フィードシナリオストアのクリア | なし |
...
RSS/Atom Feeds  -->  Fetcher  -->  Analyzer  -->  Store  -->  SimEngine
  (8ソース)       (httpx非同期)  (パターン     (~/.infrasim/  (自動マージ)
...
6. `~/.infrasim/feed-scenarios.json` に永続保存（重複排除付きマージ）
7. 次回の `infrasim simulate` 実行時にデフォルトシナリオと自動統合
...
# ライブ環境から
infrasim tf-import --dir /path/to/terraform
...
```bash
infrasim feed-update --model infrasim-model.json
infrasim feed-list
infrasim simulate  # フィードシナリオも自動統合
```

#### Webダッシュボード

```bash
infrasim serve --model infrasim-model.json --port 8080
...
| 特徴 | InfraSim | Gremlin | Chaos Monkey | Litmus | securiCAD |
|------|---------|---------|------|------|------|
