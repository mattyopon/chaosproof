...
FaultRayは、**本番環境に一切触れずに**インフラストラクチャの可用性上限を数学的に証明するカオスエンジニアリングプラットフォームです。EU Digital Operational Resilience Act（DORA）が2025年1月17日に施行され、金融機関はICTリスク管理フレームワークの構築を義務付けられています。FaultRayは、DORAの要求するICTリスクシナリオテスト（第26条）とデジタルオペレーショナルレジリエンステスト（第24条〜第27条）を、ゼロリスクかつ低コストで実現します。

### English

FaultRay is a chaos engineering platform that **mathematically proves your infrastructure's availability ceiling without touching production**. With the EU Digital Operational Resilience Act (DORA) enforced since January 17, 2025, financial entities are required to build ICT risk management frameworks. FaultRay enables the ICT risk scenario testing (Article 26) and digital operational resilience testing (Articles 24–27) mandated by DORA, at zero risk and low cost.
...
- **ゼロリスク**: 実際の障害を注入しない。YAMLトポロジーファイルのみで動作
...
- **Zero risk**: No real faults injected. Operates from YAML topology file only
...
| **第25条** テストツール・システムの要件 | テストが事業運営を阻害しないこと | ゼロリスク（シミュレーションのみ、本番影響なし） |
...
| **Article 25** Testing Tools & Systems Requirements | Testing must not disrupt business operations | Zero risk (simulation only, no production impact) |
