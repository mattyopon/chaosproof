...
FaultRay（旧InfraSim）を「売れるプロダクト」にするための改善を実装してください。
...
- テンプレート: `src/infrasim/api/templates/` (base.html, dashboard.html, components.html, simulation.html, graph.html, analyze.html)
- スタイル: `src/infrasim/api/static/style.css`
- グラフJS: `src/infrasim/api/static/graph.js`
...
### 1-1. ダッシュボード強化
**ファイル**: `src/infrasim/api/templates/dashboard.html`（既存を大幅改修）
...
**ファイル**: `src/infrasim/api/templates/base.html`（既存を修正）
...
#### Cost Impact ページ
**ファイル**: `src/infrasim/api/templates/cost.html`（新規）
...
#### Security ページ
**ファイル**: `src/infrasim/api/templates/security.html`（新規）
- セキュリティ耐性スコアの内訳
...
#### Compliance ページ
**ファイル**: `src/infrasim/api/templates/compliance.html`（新規）
...
#### Reports ページ
**ファイル**: `src/infrasim/api/templates/reports.html`（新規）
- 生成済みレポート一覧
- HTML/PDF/JSON/CSVダウンロード
- 定期レポート設定（将来のSaaS版向けスタブ）

#### Settings ページ
**ファイル**: `src/infrasim/api/templates/settings.html`（新規）
- プロジェクト設定（名前、通知先）
...
### 1-5. APIエンドポイント追加
**ファイル**: `src/infrasim/api/server.py`（既存を拡張）
...
### 2-1. OpsGenie 連携
**ファイル**: `src/infrasim/integrations/opsgenie.py`（新規）
...
### 2-2. Datadog 連携
**ファイル**: `src/infrasim/integrations/datadog.py`（新規）
...
### 2-3. Grafana 連携
**ファイル**: `src/infrasim/integrations/grafana.py`（新規）
...
**ファイル**: `src/infrasim/integrations/webhooks.py`（既存を拡張）
...
### 3-1. 課金基盤のスタブ
**ファイル**: `src/infrasim/api/billing.py`（新規）
...
### 3-2. マルチテナント強化
**ファイル**: `src/infrasim/api/database.py`（既存を拡張）
...
### 3-3. APIバージョニング
**ファイル**: `src/infrasim/api/server.py`（既存を修正）
...
### 5-1. インタラクティブデモ
**ファイル**: `src/infrasim/api/templates/demo.html`（既存を大幅改修）
...
### 6-1. ランディングページ
**ファイル**: `src/infrasim/api/templates/landing.html`（新規）
